...
(function (window, document, undefined) {
    ...
    _html2canvas.Util.getCSS = function (element, attribute, index) {
        ...
        if (/^background(Size|Position)$/.test(attribute)) {
            return parseBackgroundSizePosition(
                value,
                element,
                attribute,
                index
    ...
    _html2canvas.Parse = function (images, options) {
        ...
        var element =
            ...
            ignoreElementsRegExp = new RegExp(
            ...
            switch (text_decoration) {
                ...
                case "line-through":
                    ...
                    renderRect(
                        ...
                        bounds.left,
                        Math.ceil(
                            bounds.top + metrics.middle + metrics.lineWidth
                        ),
                        bounds.width,
                        1,
                        color
        ...
        function getTextBounds(state, text, textDecoration, isLast, transform) {
            ...
            if (support.rangeBounds && !transform) {
                ...
                var newTextNode = isLast
                    ? state.node.splitText(text.length)
                    : null;
        ...
        function listItemText(element, type) {
            ...
            switch (type) {
                ...
                case "lower-roman":
                    text =
                        _html2canvas.Generate.ListRoman(
                            currentIndex
                        ).toLowerCase();
                    break;
                case "lower-alpha":
        ...
        function parseBorders(element, bounds, borders) {
            var x = bounds.left,
                ...
                borderPoints = calculateCurvePoints(
                    bounds,
                    borderRadius,
                    borders
                ),
                ...
                if (boxModel === "border-box") {
                    var borderTop = parseInt(getCSS(el, "borderTopWidth"), 10),
                        borderBottom = parseInt(
                            getCSS(el, "borderBottomWidth"),
                            10
                        );
                    var lineHeightYOffset = borderTop + borderBottom;
                }
                valueWrap.style.lineHeight =
                    lineHeight - lineHeightYOffset + "px";
            }
        ...
        function getPseudoElement(el, which) {
            ...
            //strips quotes
            if (
                first === content.substr(content.length - 1) &&
                first.match(/'|"/)
            ) {
        ...
        function renderBackgroundRepeating(el, bounds, ctx, image, imageIndex) {
            var backgroundSize = Util.BackgroundSize(
                    el,
                    bounds,
                    image,
                    imageIndex
            ...
            backgroundRepeat =
                backgroundRepeat[imageIndex] || backgroundRepeat[0];
        ...
        function createStack(element, parentStack, bounds, transform) {
            ...
            if (
                ...
                /(hidden|scroll|auto)/.test(getCSS(element, "overflow")) ===
                    true &&
                /(BODY)/i.test(element.nodeName) === false
            ) {
                stack.clip = stack.clip
                    ? clipBounds(stack.clip, bounds)
                    : bounds;
            }
        ...
        function renderElement(
            ...
            switch (element.nodeName) {
                ...
                case "INPUT":
                    ...
                    if (
                        /^(text|url|email|submit|button|reset|password)$/.test(
                            element.type
                        ) &&
                ...
                case "SELECT":
                    if (
                        (element.options || element.placeholder || "").length >
                        0
                    ) {
                        renderFormValue(element, bounds, stack);
                    }
        ...
        function parseElement(element, stack, pseudoElement) {
            if (isElementVisible(element)) {
                stack =
                    renderElement(element, stack, pseudoElement, false) ||
                    stack;
            ...
            if (elNodeType === 1 || elNodeType === undefined) {
                ...
                try {
                    loadBackgroundImages(
                        Util.getCSS(el, "backgroundImage"),
                        el
                    );
                } catch (e) {
                    Util.log(
                        ...
                        if (images.hasOwnProperty(src)) {
                            ...
                            if (
                                ...
                                if (img.script && img.script.parentNode) {
                                    img.script.setAttribute(
                                        "src",
                                        "about:blank"
                                    ); // try to cancel running request
                                    img.script.parentNode.removeChild(
                                        img.script
                                    );
                                }
                                images.numLoaded++;
        ...
        function createRenderQueue(parseQueue) {
            ...
            rootContext = (function buildStackingContext(rootNode) {
                ...
                function insert(context, node, specialParent) {
                    var zi =
                            node.zIndex.zindex === "auto"
                                ? 0
                                : Number(node.zIndex.zindex),
